  ...
  while (idx != data.size())
    {
      ...
      Point A, B, C;
      A = pos+scale*Triple(data[idx].x, data[idx].y,data[idx].z);
      B = pos+scale*Triple(data[idx+1].x, data[idx+1].y,data[idx+1].z);
      C = pos+scale*Triple(data[idx+2].x, data[idx+2].y,data[idx+2].z);
